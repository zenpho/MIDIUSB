...
/// MIDI Jack EndPoint AudioControl Descriptor for a single device-to-host jack.
typedef struct
{
    uint8_t len;        // 5
    uint8_t dtype;      // 0x24=MIDI_CS_INTERFACE, 0x25=MIDI_CS_ENDPOINT
    uint8_t subtype;    // 0x1
    uint8_t embJacks;
    uint8_t jackID;
} MIDI_EP_ACDescriptor_In;

// MIDI Jack EndPoint AudioControl Descriptor used for quad host-to-device jacks.
// library zpo v1.0.7 sets values for this in 'getInterface()' directly
typedef struct
{
    uint8_t len;         // 8
    uint8_t dtype;       // 0x25 (MIDI_CS_ENDPOINT)
    uint8_t subtype;     // 1
    uint8_t embJacks;    // 2
    uint8_t jackID1;     // 1
    uint8_t jackID2;     // 2
...
typedef struct
{
    ...
    MIDI_EP_ACDescriptor_Out           MIDI_In_Jack_Endpoint_SPC;
    MIDI_EPDescriptor                  MIDI_Out_Jack_Endpoint;
    MIDI_EP_ACDescriptor_In            MIDI_Out_Jack_Endpoint_SPC;
} MIDIDescriptor;
...
// library v1.0.5 previously used a macro for both host-to-device and device-to-host jacks
// library zpo v1.0.7 now uses this macro for from-device-to-host jacks only
// and now defines (without a macro) from-host-to-device jacks directly in 'getInterface()'
#define D_MIDI_AC_JACK_EP_IN(_nMIDI, _iDMIDI) \
